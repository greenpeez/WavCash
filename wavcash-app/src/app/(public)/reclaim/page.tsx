...
.reclaim-root *, .reclaim-root *::before, .reclaim-root *::after {
...
.reclaim-root .nav-logo:hover { color: var(--accent); }
.reclaim-root .nav-logo svg { color: inherit; transition: transform 0.8s ease, color 0.8s ease; }
.reclaim-root .nav-logo:hover svg { transform: scale(1.08); }
.reclaim-root .nav-links { display: flex; gap: 0; align-items: center; }
.reclaim-root .nav-link {
  font-size: 14px; font-weight: 500; color: var(--text-secondary);
  text-decoration: none; background: none; border: none;
  position: relative; padding: 4px 16px; transition: color 0.8s ease;
  font-family: inherit;
}
.reclaim-root .nav-link::after {
  content: ''; position: absolute; bottom: -2px; left: 0;
  width: 0; height: 1.5px;
  background: var(--nav-underline);
  transition: width 0.5s ease, background 0.5s ease;
}
.reclaim-root .nav-link:hover { color: var(--accent); }
.reclaim-root .nav-link:hover::after { width: 0; }
.reclaim-root .nav-link.active { color: var(--accent); }
.reclaim-root .nav-link.active::after { width: 100%; background: var(--accent); }
.reclaim-root .nav-right { display: flex; align-items: center; gap: 0; }
.reclaim-root .nav-hit { display: flex; align-items: center; padding: 0 16px; }
.reclaim-root .theme-toggle {
...
.reclaim-root .theme-toggle:hover {
  color: var(--accent);
  border-color: rgba(212,136,58,0.35);
  background: rgba(212,136,58,0.08);
}
.reclaim-root .theme-toggle svg { width: 16px; height: 16px; transition: transform 0.5s ease; }
.reclaim-root .theme-toggle:hover svg { transform: rotate(15deg) scale(1.1); }
.reclaim-root .icon-sun { display: block; }
.reclaim-root .icon-moon { display: none; }
.reclaim-root[data-theme="light"] .icon-sun { display: none; }
.reclaim-root[data-theme="light"] .icon-moon { display: block; }
.reclaim-root .nav-cta {
...
.reclaim-root .nav-cta:hover {
  background: var(--accent); color: #000;
  transform: translateY(-1px) scale(1.03);
  box-shadow: 0 4px 20px rgba(212,136,58,0.25), 0 0 0 1px rgba(212,136,58,0.15);
}
.reclaim-root .nav-cta:active { transform: scale(0.98); }
.reclaim-root .content {
  position: relative; z-index: 1;
  flex: 1; display: flex; flex-direction: column;
  align-items: center; justify-content: center;
  text-align: center; padding: 0 40px;
}
.reclaim-root .coming-soon-label {
  font-family: inherit;
  font-size: 11px; font-weight: 700; letter-spacing: 2.5px; text-transform: uppercase;
  color: var(--accent); margin-bottom: 20px;
}
.reclaim-root .coming-soon-title {
  font-family: 'General Sans', var(--font-general-sans), sans-serif;
  font-size: clamp(36px, 6vw, 64px); font-weight: 700;
  letter-spacing: -2px; line-height: 1.05;
  color: var(--text-primary); margin-bottom: 16px;
}
.reclaim-root .coming-soon-desc {
  font-size: 16px; color: var(--text-secondary);
  line-height: 1.6; max-width: 380px;
}
@media (max-width: 768px) {
  .reclaim-root .top-nav { padding: 12px 20px; }
  .reclaim-root .nav-links { display: none; }
  .reclaim-root .nav-hit:has(.nav-cta) { display: none; }
  .reclaim-root .content { padding: 0 24px; }
}
`;
...
export default function ReclaimPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored && stored !== resolvedTheme) { setNextTheme(stored); return; }
    } catch {}
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: RECLAIM_CSS }} />
      <div ref={rootRef} className="reclaim-root">
        ...
        <nav className="top-nav">
          ...
          <div className="nav-links">
            ...
            <Link href="/reclaim" className="nav-link active">Reclaim</Link>
        ...
        <div className="content">
          <div className="coming-soon-label">Reclaim</div>
          <h1 className="coming-soon-title">Coming Soon</h1>
          <p className="coming-soon-desc">We&rsquo;re building tools to help you register with collection societies and recover uncollected royalties worldwide.</p>
        </div>
      </div>
    </>
  );
